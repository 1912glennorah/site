...
function initGui() {
  gui.width = 250;
  guiSpeed = gui.add(parameters, 'speed').min(.1).max(1).step(.01).name('Time');
  guiHue = gui.add(parameters, 'hue').min(0).max(1).step(.01).name('Mood');
  guiVariation = gui.add(parameters, 'hueVariation').min(0).max(1).step(.01).name('Number of Notes');
  guiGradient = gui.add(parameters, 'gradient').min(0).max(1).step(.01).name('Natural Scent');
  guiDensity = gui.add(parameters, 'density').min(0).max(1).step(.01).name('Projection');
  guiDisp = gui.add(parameters, 'displacement').min(0).max(1).step(.01).name('Depth');
